...
function getOffset(seed: number, index: number, intensity: number): number {
...
export function drawRoughRect(
  ...
  for (let i = 1; i <= rightSegments; i++) {
    const px = x + width + getOffset(seed, 200 + i, intensity);
  ...
  for (let i = 1; i <= bottomSegments; i++) {
    ...
    const py = y + height + getOffset(seed, 400 + i, intensity);
    ctx.lineTo(px, py);
  }
  ...
  for (let i = 1; i <= leftSegments; i++) {
    const px = x + getOffset(seed, 600 + i, intensity);
